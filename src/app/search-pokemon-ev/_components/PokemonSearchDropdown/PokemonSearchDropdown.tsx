    ...
    >
      {pokemons.map((pokemon, index) => (
        <div
          ...
          className={`px-4 py-3 cursor-pointer hover:bg-gray-100 border-b border-gray-100 last:border-b-0 transition-colors duration-150 h-[120px]
              ...
              {pokemon.evStats?.map((evStat) => {
                return STAT_LABELS.map((stat) => {
                  if (stat.statName === evStat.statName) {
                    return <p key={stat.statName}>{`${stat.label} +${evStat.evValue}`}</p>;
                  }
                });
              })}
            </div>
